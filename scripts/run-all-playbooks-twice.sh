...
resume_and_check_slurm_compute() {
  log "Resume compute01 and verify node state via sinfo"
  "${ANSIBLE_ADHOC_BIN}" head -i "${INVENTORY}" "${VAULT_ARGS[@]}" -b -m shell -a \
    "sudo scontrol update NodeName=compute01 State=RESUME && sudo sinfo -N -n compute01"
}
...
main() {
  ...
  for pass in 1 2; do
    ...
    for playbook in "${PLAYBOOKS[@]}"; do
      ...
      if [[ "${playbook}" == "05-slurm-compute.yml" ]]; then
        resume_and_check_slurm_compute
      fi
    done
  done
