...
Chính sách truy cập hiện tại:

- `/proj`: nhóm `g_stu` và `g_lec` có quyền đọc/ghi, `g_guest` sẽ không có quyền.
- `/soft`: chỉ đọc với user thường; cài đặt/cập nhật phần mềm do Admin thực hiện trên `storage01`.
...
| `srun ...` | Chạy lệnh/job trực tiếp qua scheduler (foreground) |
...
### 5.8 Dùng `srun` Khi Nào?

`srun` phù hợp cho các tác vụ chạy trực tiếp qua Slurm mà không cần viết file script.

- Dùng `sbatch`: khi chạy batch, dài hạn, muốn lưu script cố định.
- Dùng `srun`: khi test nhanh, hoặc chạy interactive shell trên compute node.

```bash
srun -A {user-group} -p small {slum-partition} --x11(--pty) /path-to-app
```

Ví dụ:

```bash
# Bước 1: SSH vào login node hoặc login qua X2Go

# Bước 2: Chạy app trên compute node qua Slurm
# Đối với GUI App:
srun -A g_stu -p small --x11 /app-name

# Đối với Terminal-interactive App:
srun -A g_stu -p small --pty /app-name

Checklist nếu GUI không hiện cửa sổ:

- SSH vào login node phải dùng `-X` hoặc `-Y`.
- Kiểm tra biến `DISPLAY`: `echo $DISPLAY` (không được rỗng).
- Dùng `srun --x11 ...` thay vì chạy trực tiếp app trên login node.

Nếu `srun` bị `PD` (pending), kiểm tra lý do bằng:

```bash
squeue -u $USER -o "%.10i %.9P %.8T %.20R"
```

Nếu báo lỗi account/partition, kiểm tra lại `-A` và `-p` đúng policy hiện tại của cụm.
