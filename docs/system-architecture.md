...
Hệ thống **HPC Semi-Lab** là một cụm tính toán hiệu năng cao (HPC) quy mô nhỏ, được xây dựng cho mục đích **giảng dạy và thực hành nghiên cứu**. Hệ thống cung cấp môi trường tính toán chia sẻ, quản lý tập trung, cho phép sinh viên và giảng viên thực hiện các tác vụ tính toán song song và lưu trữ dữ liệu khoa học.
...
- **Workload:** Bài toán tính toán song song, chạy phần mềm, kiểm thử bán dẫn
- **Môi trường:** Semi-lab — phù hợp giáo dục
...
| Hostname    | IP Address     | Role           | Mô Tả                                                      |
|-------------|----------------|----------------|------------------------------------------------------------|
...
- Tài nguyên mỗi node: **12 CPU cores**, **30 GB RAM** (cấu hình mặc định, override được trong inventory)
- NFS client: mount `/home`, `/proj` (rw) và `/soft` (ro) từ storage01
...
| `/soft`    | storage01  | Phần mềm khoa học, môi trường conda/spack         | head01 (rw); login01, compute* (ro)    | rw/ro |
...
Tất cả mount NFS sử dụng tùy chọn chuẩn: `rw,hard,intr,nfsvers=3,tcp,rsize=1048576,wsize=1048576`

| Tùy Chọn              | Ý Nghĩa                                                          |
|-----------------------|------------------------------------------------------------------|
| `hard,intr`           | Tự động kết nối lại khi NFS server gián đoạn tạm thời           |
| `nfsvers=3`           | NFSv3 — đã kiểm tra tương thích với Rocky Linux 8               |
...
**Cơ chế hoạt động:** Khi sử dụng đạt soft limit → hệ thống cảnh báo và bắt đầu đếm grace period. Khi vượt hard limit → hệ thống từ chối ghi thêm dữ liệu.
...
Tất cả mật khẩu và giá trị nhạy cảm được đọc từ **biến môi trường runtime** — không được commit vào repository. File `.env` chứa các biến này, được `source` trước khi chạy Ansible.

| Biến Môi Trường            | Dùng Bởi                                     | Mô Tả                                                               |
|----------------------------|----------------------------------------------|---------------------------------------------------------------------|
| `LDAP_BIND_PASSWORD`       | Ansible roles `ldap-server`, `ldap-client`   | Password của `cn=Directory Manager` — quyền tối cao 389-ds         |
| `LDAP_TEST_USER_PASSWORD`  | `identity-ldap` role                         | Password user test `hpc.test` (fallback về `LDAP_BIND_PASSWORD`)   |
| `LDAP_USERADMIN_PASSWORD`  | `create/delete-hpc-user.sh`                  | Password của `cn=hpc-useradmin` — scoped service account, **không** phải Directory Manager |
...
| `SMTP_HOST`                | `create-hpc-user.sh`                         | SMTP server gửi email welcome (ví dụ `smtp.gmail.com`)              |
| `SMTP_PORT`                | `create-hpc-user.sh`                         | SMTP port — mặc định `587` (STARTTLS)                               |
| `SMTP_USER`                | `create-hpc-user.sh`                         | Tài khoản đăng nhập SMTP                                            |
| `SMTP_PASSWORD`            | `create-hpc-user.sh`                         | Password SMTP                                                        |
| `SMTP_FROM`                | `create-hpc-user.sh`                         | Địa chỉ gửi trong email (mặc định = `SMTP_USER`)                    |

File `.env.example` được commit làm template tham khảo (không chứa giá trị thật). Xem chi tiết tại `.env.example`.
...
Hiện tại xác thực qua **LDAP simple bind** trên kênh STARTTLS. Chưa có Kerberos/GSSAPI.
...
| Thành Phần    | Hiện Tại                                    | Roadmap / Kế Hoạch                     |
|---------------|---------------------------------------------|----------------------------------------|
| LDAP          | **StartTLS bật** — `ldap://:389` + STARTTLS | Upgrade lên `ldaps://:636` nếu cần    |
| NFS           | Cleartext (NFSv3)                           | NFS over TLS (v4.1+) hoặc VPN overlay |
| Slurm (MUNGE) | HMAC-SHA1 credential                        | Giữ nguyên — phù hợp semi-lab         |
| SSH           | OpenSSH (mã hóa transport mặc định)         | Đã bật, đạt yêu cầu                   |
| Grafana       | HTTP                                        | Bật HTTPS với self-signed cert         |
| Prometheus    | HTTP                                        | Restrict access bằng firewall rule     |
...
Chỉ LDAP account có `uidNumber ≥ 10000` (user HPC thực sự) mới được phép đăng nhập hệ thống. System account bị chặn ở tầng SSSD, không chỉ ở PAM.
...
├── .env                              ← Secrets (KHÔNG commit — xem .env.example)
├── ansible.cfg                       ← Ansible config chung
├── inventory/dev/
│   ├── hosts.yml                     ← Danh sách node và IP address
│   └── group_vars/
│       ├── all.yml                   ← Biến toàn cụm (LDAP, Slurm, groups, NFS paths)
...
### 8.2 Biến Quan Trọng trong `group_vars/all.yml`
